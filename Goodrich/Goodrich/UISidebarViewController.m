...
@interface UISidebarViewController () <UIGestureRecognizerDelegate>
    ...
    /** For detecting tap and pan on center when sidebar is showing */
    @property (nonatomic, strong) UITapGestureRecognizer *tapGestureForOverlay;
    @property (nonatomic, strong) UIPanGestureRecognizer *panGestureForOverlay;
...
- (id)initWithCenterViewController:(UIViewController*)center andSidebarViewController:(MenuViewController *)sidebar
{
    ...
    if (self)
    {
        ...
        appDelegate.sidebarViewControllerDelegate = self;
...
- (void)viewDidLoad
{
    ...
    [self.overlayView addGestureRecognizer:self.panGestureForOverlay];
    [self.overlayView addGestureRecognizer:self.tapGestureForOverlay];
    
    // Also, add the pangesture to the left menu:
    [self.sidebarVC.view addGestureRecognizer:self.closeSidebarPanGesture];
}
...
- (void)setupGestures
{
    ...
    self.tapGestureForOverlay = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(viewTapped:)];
    
    // Pan gesture for overlay
    self.panGestureForOverlay = [[UIPanGestureRecognizer alloc] initWithTarget:self action:@selector(viewPanned:)];
    [self.closeSidebarPanGesture setMinimumNumberOfTouches:1];
    [self.closeSidebarPanGesture setMaximumNumberOfTouches:1];
    [self.closeSidebarPanGesture setDelegate:self];
}
...
- (void)viewPanned:(UIPanGestureRecognizer *)gesture
{
    ...
    if (gesture.state == UIGestureRecognizerStateChanged)
    {
        ...
        // If the sidebarVC is not shown in full,
        // Allow dragging only in x-coordinates by only updating the x-coordinate with translation position.
        if (self.sidebarVC.view.frame.origin.x < self.sidebarWidth - self.sidebarVC.view.frame.size.width ||
            velocity.x < 0) {
...
{
    return self.sidebarEnabled;
}

#pragma mark - SidebarItemClickedDelegate

- (void) sidebarItemClickedWithNewViewController: (UIViewController*)vc{
    
    [self displaySidebar:NO animations:nil completion:nil];
    
    if (self.centerVC.navigationController == nil && (![self.centerVC isKindOfClass:[UINavigationController class]])) {
        [self.centerVC presentViewController:vc animated:YES completion:nil];
    } else{
        UINavigationController* navVC = nil;
        
        if ([self.centerVC isKindOfClass:[UINavigationController class]]){
            navVC = (UINavigationController*)self.centerVC;
        } else{
            navVC = self.centerVC.navigationController;
        }
...
#pragma mark - ToggleSideBarButtonClickedDelegate

- (void)toggleSideBarButtonClicked{
    [Flurry logEvent:@"sidebar button clicked"];
    [self toggleSidebar:nil];
}

- (void) setSideBarEnabled: (BOOL)enabled{
    self.sidebarEnabled = enabled;
}
